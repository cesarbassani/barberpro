...
export function CanceledOrdersReport() {
    ...
    if (!ordersLoading && orders) {
      ...
      const filteredOrders = orders.filter(order => {
        ...
        return (
          ...
          orderDate >= filterValues.startDate &&
          orderDate <= filterValues.endDate
        );
      });
  ...
  }, [orders, ordersLoading, filterValues]);
  ...
  const handleApplyFilters = () => {
    ...
    // Update filter values to reflect current state
    setFilterValues({
      startDate,
      endDate,
      searchTerm,
  ...
  // Filter orders based on current filter values and search
  const getFilteredOrders = () => {
    if (!cancelledOrders) return [];
    
    return cancelledOrders.filter(order => {
      // Apply operator filter
      if (filterValues.operator !== 'all' && order.barber?.id !== filterValues.operator) {
        return false;
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="flex items-end">
            <button
              ...
              className="h-10 w-full flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50"
            >
              {isApplyingFilters ? (
